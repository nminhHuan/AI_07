...
def max_value(state, depth):
    ...
    for action in p_moves(x, y):
        ...
        v2, a2 = min_value(next_state, depth - 1)
        if v2 > v:
            v = v2
            best_move = action
    return v, best_move
...
def min_value(state, depth):
    ...
    for action in p_moves(x, y):
        next_state = get_result(state, action)
        v2, a2 = max_value(next_state, depth - 1)
        if v2 < v:
            v = v2
            best_move = action
    return v, best_move
...
if __name__ == "__main__":
    ...
    print(f"\nBẮT ĐẦU TÌM ĐƯỜNG VỚI ĐỘ SÂU (DEPTH) = {depth_limit}")
    while not states_equal(current_state, goal):
        ...
        for action in p_moves(x, y):
            ...
            v, _ = min_value(next_state, depth_limit - 1)
            if v > max_v:
                max_v = v
                best_move = action      
        if best_move is None:
            best_move = minimax_search(current_state, depth_limit)
