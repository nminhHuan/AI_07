...
class PuzzleGUI:
    ...
    def create_widgets(self):
        ...
        tk.Label(header_frame, text="using Iterative Deepening Search (IDS)",
    ...
    def start_solving(self):
        ...
        self.status_label.config(text="Thuật toán IDS đang tìm đường, vui lòng đợi...",
        ...
        if path is not None:
            self.status_label.config(
                ...
                fg=self.primary_color)
            self.animate_solution(path, depth_found)
        else:
    ...
    def animate_solution(self, path, depth_found):
        ...
        def step(index):
            if index < len(path):
                ...
                self.status_label.config(
                    text=f"Giải xong! {len(path)} bước tại độ sâu {depth_found}. (GOAL) ✓",
