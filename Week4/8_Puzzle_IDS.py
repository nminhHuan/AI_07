...
def depth_limited_search(start_state, goal_state, limit):
    initial_tuple = state_to_tuple(start_state)
    frontier = [(start_state, [], 0, {initial_tuple})]
    result_status = "failure" 
    while len(frontier) > 0:
        node_state, path, depth, ancestors = frontier.pop() 
        ...
        if depth > limit:
            ...
            for action in reversed(moves):
                ...
                if child_tuple not in ancestors:
                    ...
                    frontier.append((child_state, path + [action], depth + 1, new_ancestors))
...
def iterative_deepening_search(start_state, goal_state):
    ...
    for depth in range(0, 1000): 
        result_path, result_status = depth_limited_search(start_state, goal_state, depth)
...
class PuzzleGUI:
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(header_frame, text="8-Puzzle Solver", 
                               font=(self.font_family, 24, "bold"), 
                               bg=self.bg_color, fg=self.primary_color)
        title_label.pack()
        
        algo_label = tk.Label(header_frame, text="using Iterative Deepening Search (IDS)", 
                              font=(self.font_family, 12), 
                              bg=self.bg_color, fg=self.text_color_sub)
        algo_label.pack(pady=(5, 0))
        ...
        self.status_label = tk.Label(status_frame, text="Trạng thái ban đầu. Hãy bấm 'Giải toán'!", 
        ...
        self.solve_btn = tk.Button(btn_frame, text="Giải toán (IDS)", 
            ...
            for j in range(3):
                ...
                if val == 0:
                    ...
                    self.buttons[i][j].config(text=str(val), 
                                              bg=self.tile_color, fg=self.tile_text_color, state="normal") 
    ...
    def start_solving(self):
        ...
        self.status_label.config(text="Thuật toán IDS đang tìm đường, vui lòng đợi...", fg=self.primary_color)
        self.root.update()

        path, depth_found = iterative_deepening_search(self.current_state, goal)

        if path is not None:
            self.status_label.config(text=f"Đã tìm ra đường! Đang mô phỏng {len(path)} bước...", 
    ...
    def animate_solution(self, path):
        ...
        def step(index):
            if index < len(path):
                ...
                current_puzzle[x][y], current_puzzle[nx][ny] = current_puzzle[nx][ny], current_puzzle[x][y]
                self.current_state = copy.deepcopy(current_puzzle)
                self.update_board()
                
                self.status_label.config(text=f"Bước {index + 1}/{len(path)}: Di chuyển ô trống sang {action}", fg=self.primary_color)
                
                self.root.after(600, step, index + 1)
            else:
                self.status_label.config(text=f"Giải xong tại độ sâu {len(path)}! (GOAL)", fg=self.success_color)
                self.solve_btn.config(state="normal", bg=self.primary_color)
                self.reset_btn.config(state="normal", bg=self.error_color)
