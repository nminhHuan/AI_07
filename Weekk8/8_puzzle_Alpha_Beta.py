...
def alpha_beta_search(state, depth):
    value, best_move = max_value(state, depth, -math.inf, math.inf)
    return best_move
...
def max_value(state, depth, alpha, beta):
    ...
    for action in p_moves(x, y):
        ...
        v2, a2 = min_value(next_state, depth - 1, alpha, beta)
        if v2 > v:
            v = v2
            best_move = action
            alpha = max(alpha, v)
...
def min_value(state, depth, alpha, beta):
    ...
    for action in p_moves(x, y):
        next_state = get_result(state, action)
        v2, a2 = max_value(next_state, depth - 1, alpha, beta)
        if v2 < v:
            v = v2
            best_move = action
            beta = min(beta, v)
...
if __name__ == "__main__":
    ...
    print(f"\nBẮT ĐẦU TÌM ĐƯỜNG VỚI ALPHA-BETA (DEPTH = {depth_limit})")
    while not states_equal(current_state, goal):
        ...
        for action in p_moves(x, y):
            ...
            v, _ = min_value(next_state, depth_limit - 1, alpha, beta)
            if v > max_v:
                max_v = v
                best_move = action     
            alpha = max(alpha, max_v)
        if best_move is None:
            best_move = alpha_beta_search(current_state, depth_limit)
        print(f"MAX: {best_move} (Đánh giá điểm: {max_v})")
        current_state = get_result(current_state, best_move)
        for row in current_state:
            print(row)     
        previous_action = best_move
        if step >= 20: 
            print("\nĐã đạt giới hạn 20 bước.")
            break
